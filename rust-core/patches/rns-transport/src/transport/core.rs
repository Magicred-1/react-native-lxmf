...
impl Transport {
    pub fn new(config: TransportConfig) -> Self {
        let (announce_tx, _) = tokio::sync::broadcast::channel(256);
        let (link_in_event_tx, _) = tokio::sync::broadcast::channel(256);
        let (link_out_event_tx, _) = tokio::sync::broadcast::channel(256);
        let (received_data_tx, _) = tokio::sync::broadcast::channel(256);
        let (iface_messages_tx, _) = tokio::sync::broadcast::channel(512);
        let (resource_events_tx, _) = tokio::sync::broadcast::channel(256);
