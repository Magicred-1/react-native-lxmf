...
fn received_event_source_matches_transport_source_addr() {
    ...
    let event = lxmf_event_from_bytes(transport_src, payload, None).expect("decodable");
    match event {
...
fn two_senders_produce_distinct_reply_addresses() {
    ...
    let ev_a = lxmf_event_from_bytes(src_a, payload.clone(), None).expect("decodable");
    let ev_b = lxmf_event_from_bytes(src_b, payload, None).expect("decodable");
